...
- [ ] T018 [P] [US1] Unit tests: `@file` expansion (resolve relative to cwd, reject binary, enforce `max_file_bytes`, honor gitignore, leave unresolved `@refs` visible) in `crates/shap-core/src/files.rs`
- [ ] T019 [P] [US1] Unit test: base prompt composition in `crates/shap-core/src/prompt.rs`

### Implementation for User Story 1

- [ ] T020 [P] [US1] Implement the agent registry (configured agent → launchable process spec) in `crates/shap-agent/src/registry.rs` (depends on T008, T009)
- [ ] T021 [US1] Implement the ACP wrapper over tokio child-process stdio, implementing `AgentClient` (start session, send, stream) in `crates/shap-agent/src/acp.rs` (depends on T008, T020)
- [ ] T022 [P] [US1] Implement session-id ↔ ACP session mapping in `crates/shap-agent/src/session.rs` (depends on T008)
- [ ] T023 [P] [US1] Implement the JSONL session store (create file, append `session_started`/`user_prompt`/`agent_response`/`error`, tolerate corrupt trailing lines) in `crates/shap-core/src/session.rs` (depends on T006, T007)
- [ ] T024 [P] [US1] Implement `@file` detection/resolution/guards in `crates/shap-core/src/files.rs` (depends on T009)
- [ ] T025 [P] [US1] Implement base prompt composition (prompt + attachments) in `crates/shap-core/src/prompt.rs` (depends on T024)
- [ ] T026 [P] [US1] Implement output rendering — streamed and spinner-then-final (`indicatif`/`anstream`) — in `crates/shap-shell/src/render.rs` (depends on T007)
